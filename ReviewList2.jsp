    ...
    <div class="content_area">
        ...
        <div class="info_text_grp">
            <div class="info_cont_box">
                <div class="info_cont_tbox">
					<div class="info_cont_left">
						<button class="btn btn-edit"><a href="<%=request.getContextPath()%>/ReviewWriteForm.do">후기 작성</a></button>
					</div>
				<form name="search" >
					<div class="srch_box" style="padding: 0;">
					<fieldset class="chk_box">
						<span class="sea_ibox">
							<select name="room_type" style="width:100px; left:10px;padding-left:8px; padding-right:8px; padding-bottom:5px; " placeholder="후기 유형">
								<option value="">방 타입</option>
						...
						<span class="sea_ibox">
							<select name="search" style="width:100px; left:10px;padding-left:8px; padding-right:8px; padding-bottom:5px; ">
								...
								<option value="review_subject_review_content">제목+본문</option>
							</select>
						</span> 
						<span class="sea_ibox">
                ...
                <div class="info_ntb_box">
                    ...
                    <span class="divider">|</span>
					    ...
					    <c:forEach var="article" items="${articleList}">
					      ...
					      <%-- <c:if test="${article.mem_id ne '관리자'}">
					   		<tr height="30" class="qna_hover">
					   	 </c:if>
					   	 
					   	  <c:if test="${article.mem_id eq '관리자'}">
                       ...
                       <!-- 하나씩 감소하면서 출력하는 게시물번호 -->
								    ...
								    <td class="qna_td" align="center" style="cursor:pointer;">
									 ...
									 <c:if test="${article.review_re_level > 0 }">
										 <c:forEach begin="1" end="${article.review_re_level}">
										 &nbsp;&nbsp;
										 </c:forEach>
									  		RE :
										</c:if>
										
									  	<a href="ReviewContent.do?review_num=${article.review_num}&pageNum=${pgList.currentPage}">
								           ${article.review_subject}</a>
								    </td>
								       <td class="qna_td"  align="center"  width="150">
								       	${article.mem_id} 
								       </td>
								    <td class="qna_td"  align="center"  width="150">
								   		 <fmt:formatDate value="${article.review_date}"
								   		 							timeStyle="medium"
								   		 							pattern="yy.MM.dd" />
								    </td>
									         ...
									         <%-- <%System.out.println("article.mem_id=>"+article.getMem_id()); %>                                          --%>
                </div>
                </c:if>
            ...
            <ul class="pageing paginate paginate_regular">
            		...
            		<li>
            		 <a href="/ParaisoResort/ReviewList2.do?pageNum=${pgList.startPage-pgList.blockSize}&room_type=${room_type}&search=${search}&searchtext=${searchtext}"
            		 class="arrow_prev arrow pre pagebtn"></a>
                   </c:if>
                  
                   <c:forEach var="i" begin="${pgList.startPage}" end="${pgList.endPage}">
     				<a href="/ParaisoResort/ReviewList2.do?pageNum=${i}&room_type=${room_type}&search=${search}&searchtext=${searchtext}">
